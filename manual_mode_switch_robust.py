...
def manual_mode_switch_robust():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # 等待输入框出现
            log.info("等待输入框...")
            page.wait_for_selector('[contenteditable="true"][role="textbox"]', timeout=20000)
            ...
            if model_btn is None or not model_btn.is_visible():
                log.error("模式选择器未找到")
                return
            ...
            thinking_btn = page.locator("gem-menu-item").nth(1)  # 索引1是"3.5 思考 解决复杂问题"
